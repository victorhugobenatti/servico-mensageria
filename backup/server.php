...
while (true) {
	...
	if (in_array($server, $reads, true)) {
		...
		if ($new) {
			...
			fwrite($new, json_encode(["type"=>"welcome","msg"=>"Bem-vindo ao servidor de chat! Por favor, faça login com (/login seu_nome)."]) . "\n");
...
function handleMessage($id, $msg) {
	...
	switch ($msg['type']) {
		case 'login':
			...
			fwrite($stream, json_encode(["type"=>"login_ok", "msg"=>"Login bem-sucedido!\nAs funcionalidades disponíveis são:\n- Enviar mensagem: /msg USER TEXTO\n- Criar grupo: /creategroup NOME_DO_GRUPO USER1 USER2 ...\n- Enviar arquivo: /sendfile USER CAMINHO_ARQUIVO"]) . "\n");
		...
		case 'private':
			...
			if (isset($userMap[$to])) {
				$toId = $userMap[$to];
				fwrite($clients[$toId]['stream'], json_encode(["type"=>"private","msg"=> ucfirst($from).": $text"]) . "\n");
			}
			break;
		...
		case 'group':
			...
			while ($row = $stmt->fetch()) {
				...
				if (isset($userMap[$to])) {
					$toId = $userMap[$to];
					fwrite($clients[$toId]['stream'], json_encode(["type"=>"group","from"=>$from,"group"=>$gname,"msg"=>$text]) . "\n");
				}
			}
			break;
